...
class LdapBase:
    ...
    def listaGrupos(self, campos: object = ldap3.ALL_ATTRIBUTES) -> object:
        res = self.conn.search( LdapBase.grupos,
                                '(cn=*)',
                                attributes=campos )
        if res: return self.conn.entries
...
class LdapAdmin(LdapBase):
    ...
    def mudaGrupo(self, login, grupo):
        alvo = "uid={0},{1}".format(login, LdapBase.usuarios)
        atributos = {}
        atributos['gidNumber'] = [(ldap3.MODIFY_REPLACE, self.buscaGidGrupo(grupo))]
        #print(str(atributos))
        res = self.conn.modify( alvo, atributos )
        return res
